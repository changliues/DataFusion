...
source("DataFusion/proximal_model_specification_lunch.R")
proximal_model_spec <- readRDS(file = "proximal_model_spec_lunch.rds")
  ...
  methods = list(
    ...
    estimate_h = function(fold = 0) {
      ...
      pM.aZo <- function(a,x1,x2,x3){
        ...
        t(predict(pm.azo.fit, newdata = data.frame(a=a,z=c(0,1),x1=x1,x2=x2,x3=x3), type = "probs"))
    ...
    estimate_q = function(fold = 0) {
      ...
      q_mod <- function(z,a,x1,x2,x3){
        ...
        result <- c()
        for(i in 1:n){
          result <- c(result,(pa.Mo(a.vec[i],x1[i],x2[i],x3[i]) %*% pinv(pZ.aMo(a.vec[i],x1[i],x2[i],x3[i]),tol = 1e-30))[z[i]+1])
        }
