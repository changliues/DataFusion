...
ProxCIData <- setRefClass(
  ...
  methods = list(
    initialize = function(..., X = matrix(), Z = numeric(), M = numeric(), M.fac = numeric(), Y = numeric(), A = numeric(), G = numeric(), Xs = matrix()) {
      .self$X <<- X
      .self$Z <<- Z
      .self$M <<- M
      .self$M.fac <<- M.fac
    ...
    r1 = function() {
      return(cbind(unvec(.self$M.fac), unvec(.self$A), unvec(.self$X)))
    },
